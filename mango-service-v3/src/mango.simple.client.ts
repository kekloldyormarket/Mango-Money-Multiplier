...
class MangoSimpleClient {
  ...
  static async create() {
    ...
    const clusterUrl =
      process.env.CLUSTER_URL || "https://solana--mainnet--rpc.datahub.figment.io/apikey/995d9d62662252c679a6e673fb31b392i";
  ...
  //  process.env.MANGO_ACCOUNT = 'AdrDtyRdVALYifgoK5qGD3g7siF4Gc6aH1f8gcqxrWPY';
    if (process.env.MANGO_ACCOUNT) {
      ...
      logger.info(
        `- fetching mango accounts for ${pubkey}`
      );
      let mangoAccounts;
      try {
        ...
        logger.error(
          `- error retrieving mango accounts for ${pubkey}`
        );
        process.exit(1);
      }

      if (!mangoAccounts.length) {
        logger.error(`- no mango account found ${pubkey}`);
        process.exit(1);
      }
