...
class DataMapper {
    _options;
    _bidsAccountOrders = undefined;
    _asksAccountOrders = undefined;
    _bidsAccountPerpOrders = undefined;
    _asksAccountPerpOrders = undefined;
    // _local* are used only for verification purposes
    _localBidsOrdersMap = undefined;
    _localAsksOrdersMap = undefined;
    _initialized = false;
    _lastSeenSeqNum = undefined;
    _l3SnapshotPublishRequested = false;
    _currentL2Snapshot = undefined;
    _currentQuote = undefined;
    _version;
    _zeroWithPrecision;
    _recentTrades = new helpers_1.CircularBuffer(100);
    ...
    *map({ accountsData, slot }) {
        ...
        const newAsksSlabItems = accountsData.asks !== undefined
            ? [...new mango_client_1.BookSide(undefined, this._options.market, mango_client_1.BookSideLayout.decode(accountsData.asks)).items()]
        ...
        const newBidsSlabItems = accountsData.bids !== undefined
            ? [...new mango_client_1.BookSide(undefined, this._options.market, mango_client_1.BookSideLayout.decode(accountsData.bids)).items()]
        ...
        if (this._initialized) {
            ...
            if (diffIsValid === false) {
                logger_1.logger.log('warn', 'Invalid l3diff', {
                    market: this._options.symbol,
        ...
        const shouldInitialize = this._initialized === false && this._asksAccountOrders !== undefined && this._bidsAccountOrders !== undefined;
        const snapshotHasChanged = this._initialized === true && (accountsData.asks !== undefined || accountsData.bids !== undefined);
        if (shouldInitialize || snapshotHasChanged || this._l3SnapshotPublishRequested) {
            ...
            if (isInit && accountsData.eventQueue !== undefined) {
                ...
                const header = mango_client_1.PerpEventQueueHeaderLayout.decode(accountsData.eventQueue);
        ...
        const newL2Snapshot = {
            ...
            bids: accountsData.bids !== undefined
        ...
        const bookIsCrossed = newL2Snapshot.asks.length > 0 &&
        ...
        const asksDiff = accountsData.asks !== undefined ? this._getL2Diff(this._currentL2Snapshot.asks, newL2Snapshot.asks) : [];
            ...
            for (let i = 0; i < l3Diff.length; i++) {
                ...
                if (message.type === 'fill' && message.maker === false) {
                    ...
                    if (makerFillOrderId === undefined) {
                        logger_1.logger.log('warn', 'Trade without matching maker fill order', {
                            market: this._options.symbol,
                            slot,
                            fill: message,
                            l3Diff
        ...
        if (asksDiff.length > 0 || bidsDiff.length > 0) {
            ...
            const quoteHasChanged = this._l2LevelChanged(this._currentQuote.bestAsk, newQuote.bestAsk) ||
    ...
    _addNewAndChangedOrderItemsToL3Diff(matchingExistingOrder, newOrder, timestamp, slot, l3Diff) {
        if (matchingExistingOrder === undefined) {
            ...
            if (matchingFills.length > 0) {
                for (const matchingFill of matchingFills) {
                    ...
                    size = (Number(size) + Number(matchingFill.size)).toFixed(this._options.sizeDecimalPlaces);
                }
            }
        ...
        else if (matchingExistingOrder.size !== newOrder.size &&
    ...
    _addDoneOrderItemsL3Diff(doneOrderItem, timestamp, slot, l3Diff) {
        let reason;
        let sizeRemaining;
        const matchingFills = l3Diff.filter((f) => f.type === 'fill' && f.maker === true && f.orderId === doneOrderItem.orderId);
        if (matchingFills.length > 0) {
        ...
        for (const item of l3Diff) {
            ...
            if (item.type === 'fill') {
                ...
                if (matchingOrder !== undefined) {
                    ;
                    matchingOrder.size = (Number(matchingOrder.size) - Number(item.size)).toFixed(this._options.sizeDecimalPlaces);
                }
                else if (item.maker === true) {
                    logger_1.logger.log('warn', 'Maker fill without open message', {
                        market: this._options.symbol,
                        fill: item,
                        slot
                    });
    ...
    _getL2Diff(currentLevels, newLevels) {
        ...
        for (const levelToRemove of currentLevelsMap) {
            const l2Delete = [levelToRemove[0], this._zeroWithPrecision];
            l2Diff.unshift(l2Delete);
        }
        return l2Diff;
    }
    _l2LevelChanged(currentLevel, newLevel) {
    ...
    _toMapConstructorStructure(orderItem) {
        return [orderItem.orderId, orderItem];
    }
    *_mapEventToDataMessages(eventInfo, timestamp, slot) {
    ...
    *_getNewlyAddedEvents(eventQueueData) {
        const header = mango_client_1.PerpEventQueueHeaderLayout.decode(eventQueueData);
        ...
        if (this._lastSeenSeqNum !== undefined) {
            const allocLen = Math.floor((eventQueueData.length - mango_client_1.PerpEventQueueHeaderLayout.span) / mango_client_1.PerpEventLayout.span);
            ...
            for (let i = newEventsCount; i > 0; --i) {
                ...
                const decodedItem = mango_client_1.PerpEventLayout.decode(eventQueueData, mango_client_1.PerpEventQueueHeaderLayout.span + nodeIndex * mango_client_1.PerpEventLayout.span);
                if (decodedItem.fill) {
                    const fillEvent = { eventType: 'fill', data: decodedItem.fill };
                    yield fillEvent;
                }
                else if (decodedItem.liquidate) {
                    const liquidateEvent = { eventType: 'liquidate', data: decodedItem.liquidate };
    ...
    _mapToOrderMessage({ orderId, clientId, side, price, account, accountSlot, eventTimestamp }, type, size, timestamp, slot) {
        if (type === 'open') {
            return {
                type,
                market: this._options.symbol,
    ...
    _mapPerpOrderToOrderItem = ({ orderId, clientId, side, price, size, owner, openOrdersSlot, timestamp }) => {
        const orderItem = {
...
//# sourceMappingURL=data_mapper.js.map