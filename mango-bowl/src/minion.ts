...
import { MangoPerpMarketInfo } from '.'
...
import { ErrorResponse, MangoMarketListItem, RecentTrades, SubRequest, SuccessResponse } from './types'
...
class Minion {
  ...
  constructor(private readonly _nodeEndpoint: string, private readonly _markets: MangoPerpMarketInfo[]) {
        ...
        this._markets.map((market) => {
          return executeAndRetry(
            async () => {
              ...
              const mangoClient = new MangoClient(connection, new PublicKey(market.programId))
              const mangoGroup = await mangoClient.getMangoGroup(new PublicKey(market.groupPublicKey))
              ...
              const marketInfo: MangoMarketListItem = {
                name: market.name,
